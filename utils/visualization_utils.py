...
def check_boxes_and_labels_on_image_array(
        boxes,
        classes,
        scores,
        category_index,
    ...
    for box, color in box_to_color_map.items():
        ...
        width = xmax - xmin
        height = ymax - ymin
        detected_category = box_to_display_str_map[box][0].split(":")[0]
        ...
        if detected_category == category_name:
            ...
            if width > size_threshold and height > size_threshold:
                n_large_target += 1
                print("detected large", category_name, n_large_target)
            if ymin < edge_threshold:
                n_bb_on_edge += 1
            if xmin < edge_threshold:
                n_bb_on_edge += 1
            if ymax > 1 - edge_threshold:
                n_bb_on_edge += 1
            if xmax > 1 - edge_threshold:
                n_bb_on_edge += 1
            if n_bb_on_edge >= 2:
                print(category_name, "bb on edge", n_bb_on_edge)
                return False

            if width > size_threshold and height > size_threshold and n_large_target == 1:
                obj_box = box
        else:
            if width > size_threshold or height > size_threshold:
                print("detected large occlusion", box_to_display_str_map[box][0])
                return False
    if n_large_target != 1:
        print("not only one", category_name, n_large_target)
        return False
    print("passed filter")
    if bb_output_path is not None:
        bb = np.savetxt(bb_output_path, obj_box, delimiter=",")
    return True
